...
class QueryService implements QueryServiceInterface {
  ...
  public function __construct(QueryFactory $entity_query, Connection $connection, ModuleHandler $module_handler, LoggerChannelFactory $logger_factory) {
     ...
     *   The target content to find relevant content for.
     *
    ...
    /**
     * Set the vocabularies property.
     *   The vids used to filter content.
     *
     * @param array $vids
     ...
     * @return object $this
     */
    public function filterByVocabularies($vids) {
        // @TODO: test if these are arrays, also add methods to add single vocab as a string.
        if ( !$this->node ) {
     ...
     * @return object $this
     */
    public function setAllowedContentTypes($types) {
        if (is_array($types)) {
            $this->allowed = $types;
        }
        return $this;
     ...
     *
     * @return array
     *   Array of reference fields with id, target_type, and target_bundles keyed by field name.
     */
     ...
     *   Information array of relevant nodes with nid, type, and relevance count, keyed by Node id.
     */
